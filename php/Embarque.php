      ...
      <table border='1' overflow='scroll' height='100%' width='100%'  bgcolor='black'>
        ...
        <?php
          while ($fila = $result  -> fetch_assoc()) { ?>
            <tr>  <?php
              $orden = $fila['orden'];
              $query2 ="SELECT IF(WEEK(STR_TO_DATE(fechaVencida, '%d/%m/%Y')) <= WEEK(NOW()+INTERVAL 1 WEEK), 1, 0) AS res FROM analisisatrasos, embarque WHERE embarque.orden = analisisatrasos.orden AND señalEmb='0' AND embarque.orden='$orden'";
              ...
              if($fila2['res'] == 0) { ?>
                ...
                <?php if($fila['noCuñetes'] !='' AND $fila['pesoNeto'] !='' ) {?>
                  <td bgcolor='#90BFDE' align='center'>
                    ...
                    <button type='submit' style='background-color:#90BFDE; border-color:#90BFDE; border:0;' class='btn btn-info btn_add' onclick="this.form.action='php/EnviarA.php';this.form.submit();"><img src='img/terminado.png' width='30' height='30' class='zoom'/></button>
                     </td>
                <?php } else {?>
                  <td bgcolor='#90BFDE' align='center'>
