<?php
  ...
  $query = "SELECT embarque.orden,componente,cantidad,fechaVencida,analisisatrasos.piezasEntregar,noCuñetes,pesoNeto,Ubicacion FROM embarque, analisisatrasos WHERE embarque.orden = analisisatrasos.orden AND seEnv='1' AND señalEmb='0' ORDER BY CONCAT(SUBSTRING_INDEX(fechaVencida , '/', -1), SUBSTRING_INDEX(SUBSTRING_INDEX(fechaVencida , '/', 2), '/', -1)) <= CONVERT(DATE_FORMAT(NOW(), '%Y%m'), CHAR) DESC, embarque.orden ASC";

  //Consultar
  if (isset($_POST['consulta'])) {
    $q = $conexion -> real_escape_string($_POST['consulta']);
    $query = "SELECT embarque.orden,embarque.componente,embarque.cantidad,fechaVencida,analisisatrasos.piezasEntregar,embarque.noCuñetes,embarque.pesoNeto,embarque.Ubicacion FROM embarque,analisisatrasos WHERE embarque.orden = analisisatrasos.orden  AND señalEmb='0' AND embarque.componente LIKE '%".$q."%' AND embarque.orden = analisisatrasos.orden OR embarque.orden = analisisatrasos.orden  AND señalEmb='0' AND embarque.orden LIKE '%".$q."%' ORDER BY WEEK(STR_TO_DATE(fechaVencida, '%d/%m/%Y')) <= WEEK(NOW()) DESC";
  }
  $udu = "UPDATE ubicacion SET disponible=0 WHERE embarque.ubicacion=";
...
<html lang="en" dir="ltr">
  <head>
    ...
    <title>Almacén PT</title>
    <link rel="shortcut icon" href="/produccion/img/icono.png">
    <link rel="stylesheet" href="/css/produccion.padding" >
    ...
    <script type="text/javascript">
      $(".btn_add").on("click", function() {
        ...
        var ubicacion= $(this).closest('tr').children()[7].textContent;
        $("#orden").val(orden);
  ...
  <?php /* ?><font size=4 color='red'><n>AVISO: POR EL MOMENTO SOLO SE ENVIARÁN ORDENES VENCIDAS Y QUE VENCEN EN EL MES ACTUAL.</n> </font> <?php */ ?>
    <form  method='post'>
        ...
        <?php
          while ($fila = $result  -> fetch_assoc()) { ?>
            <tr>  <?php
              ...
              $query2 ="SELECT IF(WEEK(STR_TO_DATE(fechaVencida, '%d/%m/%Y')) <= WEEK(NOW()), 1, 0) AS res FROM analisisatrasos, embarque WHERE embarque.orden = analisisatrasos.orden AND señalEmb='0' AND embarque.orden='$orden'";
              $result2 = $conexion -> query($query2);
              $fila2 = $result2  -> fetch_assoc();

              if($fila2['res'] == 0) { ?>
                <th bgcolor='#5DADE2'><button type='submit' style='background-color:#5DADE2; border-color:#5DADE2; border:0;' class='btn btn-info btn_add' onclick="this.form.action='php/almacenEdit.php';this.form.submit();"><font size=5><n><u><?php  echo $fila['orden']; ?></u></n></font></button></th>
                ...
                <td bgcolor='#90BFDE' align='center'><font size=5><?php  echo $ubicacion; ?></font></td>
                <?php if($fila['noCuñetes'] !='' AND $fila['pesoNeto'] !='' AND $ubicacion != '') {?>
                ...
                <?php } else {?>
                  <th bgcolor='#FF4D4D'><button type='submit' style='background-color:#FF4D4D; border-color:#FF4D4D; border:0;' class='btn btn-info btn_add' onclick="this.form.action='php/almacenEdit.php';this.form.submit();"><font size=5><n><u><?php  echo $fila['orden'];?></u></n></font></button></th>
                ...
                <td bgcolor='#FF4D4D' align='center'>
                 <button type='submit' style='background-color:#FF4D4D; border-color:#FF4D4D; border:0;' class='btn btn-info btn_add' onclick="this.form.action='php/EmbarqueEnviar.php';this.form.submit();"><img src='img/terminado.png' width='30' height='30' class='zoom'/></button>
                  <!--  <button type='submit' style='background-color:#FF4D4D; border-color:#FF4D4D; border:0;' class='btn btn-info btn_add' onclick="this.form.action='php/EnviarA.php';this.form.submit();"><img src='img/terminado.png' width='30' height='30' class='zoom'/></button>-->
                </td>
                <?php } else {?>
                  <td bgcolor='#FF4D4D' align='center'>
                    <img src='img/cancelar.png' width='30' height='30'/>
      ...
      <input type="text" name="ubicacion" id="ubicacion" style="background-color:transparent; color:transparent; border-color:transparent; border-style:solid" readonly >
